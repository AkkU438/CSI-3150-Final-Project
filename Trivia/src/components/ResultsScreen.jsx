  ...
  return (
    <div className="results-screen">
      ...
      <div className={`results-hero ${revealed ? 'results-hero--visible' : ''}`}>
        ...
        <div className="grade-badge" style={{ color: grade.color, borderColor: grade.color }}>{grade.label}</div>
        <h1 className="player-name-display">{playerName || 'PLAYER ONE'}</h1>

        <div className="score-ring-wrap">
          <svg className="score-ring" viewBox="0 0 140 140" aria-hidden="true">
            <circle cx="70" cy="70" r="58" className="ring-bg" />
            <circle
              cx="70" cy="70" r="58" className="ring-fill"
        ...
        <p className="pct-display" style={{ color: grade.color }}>{pct}%</p>
      ...
      <div className={`results-lb ${revealed ? 'results-lb--visible' : ''}`}>
        <h2 className="lb-heading"><span>🏆</span> HALL OF FAME</h2>
        {leaderboard.length === 0 ? (
          <p className="lb-empty-msg">No records yet.</p>
        ) : (
          <ol className="results-lb-list">
            {leaderboard.map((entry, i) => (
              <li key={i} className={`rlb-entry ${entry.name === (playerName || 'Anonymous') && entry.score === score ? 'rlb-entry--you' : ''}`}>
